...
export const availabilityRepository = {
    async ownerBusy(from, to, profileId) {
        ...
        }, { title: 1, description: 1, location: 1, organizer: 1, participants: 1, startAt: 1, endAt: 1, allDay: 1, type: 1, color: 1, calendarId: 1 }).sort({ startAt: 1 }).lean();
    },
    async participantBusy(participantIds, from, to) {
        ...
        const events = await Event.find({
            ...
            type: { $ne: "workingLocation" },
            $or: [{ allDay: false }, { type: "outOfOffice" }],
        }, { title: 1, description: 1, location: 1, organizer: 1, participants: 1, startAt: 1, endAt: 1, allDay: 1, type: 1, color: 1, participantIds: 1, calendarId: 1 }).sort({ startAt: 1 }).lean();
