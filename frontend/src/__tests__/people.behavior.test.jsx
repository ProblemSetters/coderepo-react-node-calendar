...
import { PeoplePicker } from "../features/people/PeoplePicker.jsx";
...
const user02 = { _id: "person-1", name: "Sky", email: "sky@hackerrank.com", avatarColor: "#d93025" };
const user03 = { _id: "person-2", name: "Sage", email: "sage@hackerrank.com", avatarColor: "#1a73e8" };
const response = {
...
describe("Meet with people", () => {
    test("searches, selects, removes, and opens suggestions with the selected people", async () => {
        ...
        expect(await screen.findByRole("option", { name: /Sage/ })).toBeInTheDocument();
    ...
    test("supports listbox arrow navigation and Enter selection", async () => {
        ...
        await screen.findByRole("option", { name: /Sage/ });
        await userEvent.keyboard("{ArrowDown}{Enter}");
        expect(screen.getByRole("button", { name: "Remove Sky" })).toBeInTheDocument();
    });

    test("keeps a long people list internally scrollable and follows keyboard navigation", async () => {
        const people = Array.from({ length: 12 }, (_, index) => ({ _id: `person-${index}`, name: `Guest ${String(index + 1).padStart(2, "0")}`, email: `guest${index + 1}@hackerrank.com`, avatarColor: "#1a73e8" }));
...
describe("calendar comparison", () => {
    test("loads and renders You beside every selected person", async () => {
        ...
        expect(screen.getByLabelText("Sky schedule")).toBeInTheDocument();
        ...
        expect(screen.getByRole("button", { name: /Quick sync.*Sky/ })).toHaveAttribute("data-density", "micro");
    ...
    test("allows striped owner time but blocks striped time on another person's calendar", async () => {
        ...
        const otherSchedule = screen.getByLabelText("Sky schedule");
    ...
    test("opens busy events instead of creating a meeting through them", async () => {
        ...
        const otherEvent = await screen.findByRole("button", { name: /Design critique.*Sky/ });
        await userEvent.click(otherEvent);
        expect(create).not.toHaveBeenCalled();
        expect(select).toHaveBeenCalledWith(expect.objectContaining({ _id: "event-user02", title: "Design critique", editable: false, readOnlyOwner: "Sky", calendarName: "Sky’s calendar" }));
