...
const displayDate = (value, concise = false) => new Date(`${value}T00:00:00`).toLocaleDateString("en-US", concise
    ...
    return <div className={`editor-date-picker ${compact ? "compact" : ""} ${className}`} ref={root}>
        ...
        {open && <div className="editor-date-popover" role="dialog" aria-label={`Choose ${label.toLowerCase()}`}>
            ...
            <div className="editor-date-grid">{"SMTWTFS".split("").map((day, index) => <span className="editor-weekday" key={`${day}-${index}`}>{day}</span>)}{days.map((day) => {
                ...
                return <button aria-label={day.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })} className={`${day.getMonth() !== month.getMonth() ? "outside" : ""} ${key === value ? "selected" : ""}`} disabled={unavailable} key={key} type="button" onClick={() => { onChange(key); setOpen(false); }}>{day.getDate()}</button>;
            })}</div>
        </div>}
    </div>;
}
