    ...
    return (
        <header className="app-header">
            ...
            <div className="period-controls">
                <button className="icon-button" title="Previous" aria-label="Previous period" onClick={() => setCursor(moveCursor(view, cursor, -1))}><MaterialIcon>chevron_left</MaterialIcon></button>
                <button className="icon-button" title="Next" aria-label="Next period" onClick={() => setCursor(moveCursor(view, cursor, 1))}><MaterialIcon>chevron_right</MaterialIcon></button>
            </div><h1>{formatHeading(view, cursor)}</h1></>}
            {!searchMode && <div className="header-actions">
                <button className="icon-button" title="Search" aria-label="Search events" onClick={() => { setViewMenuOpen(false); setProfileMenuOpen(false); onSearchOpen(); }}><MaterialIcon size={23}>search</MaterialIcon></button>
